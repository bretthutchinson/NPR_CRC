...
--[PSP_GridDataReport_Get] '|AM|,|FM|', 'N', 'A', 'Regular', 1, 2014, 'N'
--select * from station where callletters = 'WSDL'
...
SET NOCOUNT ON
	...
	BEGIN
		SET @DynSql = '
		SELECT
			  ''"'' + st.CallLetters + ''-'' + b.BandName  + ''"'' As Station ,   ''"'' + convert(varchar,'  +
			CONVERT(VARCHAR, @Year) 

			IF LEN(@Month) = 1
				SET @DynSql = @DynSql + CONVERT(VARCHAR, 0) +
			--[PSP_GridDataReport_Get] '|AM|,|FM|', 'N', 'A', 'Regular', 1, 2014, 'N'
...
--TotalQHours
		...
		SET @DynSql = @DynSql+ ' Order by station'
	END
...
END
--[PSP_GridDataReport_Get] '|AM|,|FM|', 'Y', 'A', 'Regular', 6, 2014, 'Y'
--select * from station where callletters