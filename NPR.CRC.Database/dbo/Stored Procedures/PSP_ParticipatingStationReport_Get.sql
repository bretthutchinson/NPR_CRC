...
AS BEGIN
    ...
    DECLARE @DCSql NVARCHAR(MAX) = 'SELECT distinct StationID, Year, Month  FROM [CRC].[dbo].[Schedule]
     where status=''Accepted''
	  and Month in ( ' +  @CSeasonMons  + ') and year in ( ' + @currentYear + ' )'
	
	insert into #CurrentStations EXEC dbo.sp_executesql @DCSql 

	DECLARE @DPSql NVARCHAR(MAX) = 'SELECT distinct StationID, Year, Month  FROM [CRC].[dbo].[Schedule]
     where status=''Accepted''
	...
	Declare @DFinalSQL NVARCHAR(MAX) ='Select Distinct t.CallLetters + ''-'' + b.bandname as Station,
